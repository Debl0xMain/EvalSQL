...
-- change order details order_details
...
SELECT companyname AS 'Client',
orders.customerid AS 'REF_Client',
sum(order_details.quantity*order_details.unitprice) AS 'CMDTotal'
From customers
JOIN orders
ON customers.customerid=orders.customerid
JOIN order_details
...
HAVING sum(order_details.quantity*order_details.unitprice)>=30000;

--6 Liste des pays dans lesquels des produits fournis par "Exotic Liquids" ont été livrés :

SELECT DISTINCT suppliers.companyname AS 'Fournisseur',
...
SUM(order_details.unitprice*order_details.quantity) AS 'Sale amount'
FROM orders
JOIN order_details
ON order_details.orderid=orders.orderid
GROUP BY Month, Year
HAVING AVG(YEAR(orderdate)) = 1997;

--9 A quand remonte la dernière commande du client nommé "Du monde entier" ?

SELECT MAX(DATE(orderdate)) AS 'Last CMD'
FROM orders
JOIN order_details
ON order_details.orderid=orders.orderid
WHERE shipname in ('Du monde entier');

--10 Quel est le délai moyen de livraison en jours ?

SELECT ROUND(AVG(DATEDIFF(shippedDate,orderDate))) AS 'Delai de livraison moyen'
FROM orders;
